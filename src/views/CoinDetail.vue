        ...
        >
          ...
          <td>
            <px-button
              ...
              v-if="!market.url"
              @onClick="getWebsite(market)"
              >Ver</px-button
            >
            <a class="hover:underline text-green-500" v-else :href="market.url">{{ market.url }}</a>
          </td>
        </tr>
      </table>
    </template>
...
export default {
  ...
  methods: {
    getCoin() {
      ...
      Promise.all([api.getAsset(coinId), api.getAssetHistory(coinId), api.getMarkets(coinId)])
  ...
  computed: {
    ...
    avg() {
      return (
        this.history.reduce((a, item) => a + parseFloat(item.priceUsd), 0) / this.history.length
      );
    }
  },
  watch: {
